...
![2 x 2 분할표](../myimages/table22-01.png){width=60%}
```
...
![2 x 2 분할표: 관측 도수](../myimages/table22-02.png){width=60%}
```
...
귀무가설 하에서 표본의 크기가 충분히 크면 식 \@ref(eq:chisquarestat) 의 카이제곱 검정통계량 $\chi^2$ 는 
...
이제 실제 분할표에서 카이제곱 검정을 수행해 보자. 아스피린 임상실험 결과가 주어진 표 \@ref(tab:conttable25) 에서
...
R 에서도 카이제곱 검정을 쉽게 수행할 수 있다. 앞에서 표 \@ref(tab:conttable25) 의 자료를 행렬의 형태로 저장하였는데 
...
![K 개의 2 x 2 분할표](../myimages/table22-k.png){width=60%}
```
...
![8개 병원의 임상실험 결과](../myimages/beitler-data.png)
...
이제 각 병원을 층(strata)로 고려하고 병원의 효과를 제어하면서 식 \@ref(eq:cmhstat)의 검정 통계량   $Q_{CMH}$  를 이용하여 치료제의 효과가 있는지 검정해보자. 검정은 아래와 같이 
...
![짝표본 실험에 의한 2 x 2 분할표](../myimages/paired.png){width=60%}
...
![짝표본 실험에 의한 2 x 2 분할표](../myimages/paired2.png){width=60%}
```
...
$p_2$ 라고 하면 짝표본에서 얻어진 분할표 \@ref(fig:macnamar2) 에서 관심있는 가설은 다음과 같다.
...
![영국시민의 수상에 대한 지지도 조사 자료](../myimages/paired-data.png){width=80%}
```
